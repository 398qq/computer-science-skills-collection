...
> **用途**：定义 FBS-BookWriter v2.1.0 **快速起步三条路径**，帮助 AI 在首轮就把用户推进到正确阶段，降低空转追问。本文件是 Skill 的核心入口文件。
...
```
Step 1：读取进度台账
  → 优先读 .fbs/workbuddy-resume.json（v2.1.0 会话快照，含 currentStage/nextSuggested/wordCount）
