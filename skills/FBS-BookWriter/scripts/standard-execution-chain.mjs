  ...
  if (!args.bookRoot || !args.chapterId) {
    throw new Error(
      "用法: node scripts/standard-execution-chain.mjs --skill-root <技能根> --book-root <本书根> --chapter-id <章节ID> " +
        "[--mode parallel_writing|single_writer] [--gates-only] [--runtime-only] [--no-verify-stages] [--no-verify-s0-timestamp] [--no-verify-broken-links] [--no-auto-preview] [--preview-port <port>] [--preview-ttl-ms <milliseconds>]"
