...
function ensureChanged(source, output, label) {
  if (source === output) {
...
function replaceRegex(source, regex, replacer, label) {
  const output = source.replace(regex, replacer);
  return ensureChanged(source, output, label);
}
  ...
  operations.push(
    {
      ...
      apply() {
        return updateTextFile(this.filePath, (source) => {
          ...
          output = replaceRegex(output, /date: '[^']+',/, `date: '${today}',`, 'scripts/version.mjs date');
          return output;
        });
      },
    },
