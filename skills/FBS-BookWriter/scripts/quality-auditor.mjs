...
const SAFE_ADVERBS = ['非常', '显著', '大幅', '彻底', '极其', '极大地', '深刻地', '全面地', '前所未有', '充分', '尤其', '特别', '相当', '格外', '甚为', '甚'];
const PROTECTED_PHRASES = ['深度学习', '深度阅读', '深度工作', '深度理解', '高度不确定', '高度专业', '高度复杂'];
...
function collectFiles(args) {
  ...
  if (args.glob) globSync(args.glob, { cwd: args.bookRoot, absolute: true, ignore: QUALITY_SCAN_IGNORE_GLOBS }).forEach((file) => files.add(file));
...
function applyConservativeAutoFix(text) {
  const lines = text.split(/\r?\n/);
  const changes = [];
  let inFence = false;
  const nextLines = lines.map((line, index) => {
    ...
    for (const word of SAFE_ADVERBS) {
      if (PROTECTED_PHRASES.some((phrase) => phrase.includes(word) && current.includes(phrase))) continue;
      current = current.replace(new RegExp(word, 'g'), '');
    }
...
function main() {
  ...
  for (const filePath of files) {
    ...
    if (args.autoFix) {
      const fixed = applyConservativeAutoFix(text);
