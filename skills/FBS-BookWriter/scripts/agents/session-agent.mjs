...
const ROLES = {
  // 创意会角色
  RADICAL: 'radical',           // 激进派：打破常规的角度
  CONSERVATIVE: 'conservative', // 守序派：最可行最稳妥的角度
  READER: 'reader',             // 读者派：目标读者最想看的角度
  
  // 读者会角色
  NOVICE: 'novice',               // 新手读者：第一次接触本主题
  EXPERIENCED: 'experienced',     // 有基础读者：有一定了解但非专家
  EXPERT: 'expert',               // 挑剔读者：专业背景，要求很高
  
  // 对抗会角色
  SUPPORTER: 'supporter',         // 支持者：给出最强支撑论据
  OPPONENT: 'opponent',          // 反对者：给出最严苛反驳
  SYNTHESIZER: 'synthesizer',      // 综合者：在两方之间找到最强可接受表述
  
  // 评审会角色
  CONTENT_EXPERT: 'content_expert',      // 内容专家：论点与论据
  EXPRESSION_EXPERT: 'expression_expert', // 表达专家：叙事与节奏
  CRITIC: 'critic'                      // 怀疑论者：读者的批评视角
};

// 会议阶段
const PHASES = {
  DIVERGENCE: 'divergence',   // 发散阶段：各角色独立表达，不打断、不批评
  CONVERGENCE: 'convergence',   // 收束阶段：在发散结果基础上碰撞，找出最有力的综合方案
};

// 会议时长配置（单位：分钟）
const DURATION_LIMITS = {
  [SESSION_TYPES.CREATIVE]: { normal: 12.5, timeout: 20 },
...
// 心理契约选项
const PSYCHOLOGICAL_CONTRACT_OPTIONS = {
  FIND_ISSUES: 'find_issues',           // A. 发现问题（尽量多找毛病）
  BALANCE: 'balance',                   // B. 均衡（问题和亮点并重述）
  CONFIRM_HIGHLIGHTS: 'confirm_highlights' // C. 确认亮点（以鼓励为主，问题点到为止）
};
...
const SESSIONS_SUMMARY_HEADER = `# 会议纪要汇总（.fbs/sessions-summary.md）
