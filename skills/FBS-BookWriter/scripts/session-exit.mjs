...
/** 退出时第二句自然语言摘要（WorkBuddy 实测 P1-04） */
...
export async function handleSessionExit({ bookRoot, note = '', quiet = false, mirrorWorkbuddyMemory = true } = {}) {
  ...
  const snapshot = await generateSessionSnapshot({ fbsDir, quiet: true });
  const memoryResult = applyBookMemoryTemplate({ bookRoot: resolvedBookRoot, quiet: true });
  ...
  const userMessage = userSummary ? `${baseMsg}\n${userSummary}` : baseMsg;
