...
function writeEsmState(fbsDir, { from, to, reason, genre, iso }, { logger = console, quiet = false } = {}) {
  const body = `---
currentState: "${to}"
previousState: "${from}"
...
function validateTransitionInput({ from, to }) {
  const fromState = normalizeStateName(from);
  const toState = normalizeStateName(to);
  if (!VALID.has(fromState) || !VALID.has(toState)) {
    throw new Error(`状态名须为: ${[...VALID].join(", ")}`);
  }
...
export async function recordESMTransition({
  ...
  const esmStatePath = writeEsmState(
    fbsDir,
    { from: fromState, to: toState, reason, genre: normalizedGenre, iso },
    { logger, quiet },
  );
