...
function parseArgs(argv) {
  const args = { bookRoot: null, profileId: null, dryRun: false, help: false };
...
function buildResumeBrief(snapshot, bookCtx) {
  const lines = [
  ...
  lines.push('', '## 恢复指引', '');
  lines.push('AI 进入新会话时，请优先读取本文件，并在首响中说明：');
  lines.push('「已加载上次书稿记忆，当前状态：[从上方摘要提取]，继续[当前章节]的写作。」');
  lines.push('');
...
// 主流程
// ──────────────────────────────
export function applyBookMemoryTemplate({ bookRoot, profileId = null, dryRun = false, quiet = false } = {}) {
  if (!bookRoot) {
  ...
  const brief = buildResumeBrief(snapshot, bookCtx);
  ...
  if (!dryRun) {
    ...
    writeJsonAtomic(memDir, 'book-memory-snapshot.json', snapshot, { backup: true, quiet });
