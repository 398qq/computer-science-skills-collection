...
function scanLargeFiles(skillRoot, cfg) {
  const skipDirs = new Set(["node_modules", ".git", "dist", "test-unzip", "final-test", ".codebuddy", ".codebuddy-plugin"]);
  ...
  const walk = (dir) => {
    for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      const warnLines = rel === "scripts/wecom/scene-pack-admin.mjs" ? Math.max(cfg.warnLines, 1800) : cfg.warnLines;
      const failLines = rel === "scripts/wecom/scene-pack-admin.mjs" ? Math.max(cfg.failLines, 3600) : cfg.failLines;
